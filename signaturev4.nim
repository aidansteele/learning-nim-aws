...
proc signableStringv4(scope: AwsCredentialScope, request: AwsRequest): string =
  ...
  let canonicalHash = sha256(canonicalQueryv4(request))
  let signableLines = [
...
proc signingKeyv4(credentials: AwsCredentials, scope: AwsCredentialScope): string =
  ...
  let dateHmac = hmac_sha256(dateKey, date)
  let regionHmac = hmac_sha256(dateHmac, scope.region)
  let serviceHmac = hmac_sha256(regionHmac, scope.service)
  result = hmac_sha256(serviceHmac, "aws4_request") 
...
proc signaturev4*(
  ...
  result = hexify(hmac_sha256(key, signableStr))
